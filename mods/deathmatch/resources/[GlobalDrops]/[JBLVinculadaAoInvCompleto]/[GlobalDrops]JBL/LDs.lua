...
addCommandHandler('gjbl', 
    function(player)
        if not getElementData(player, 'Schootz.pegouJBL') then
			...
			exports["[GlobalDrops]InventarioDestino"]:sendItem(player, "jbl", 100, 1, "none", false)
			exports["[GlobalDrops]InventarioDestino"]:sendNotification(player, "success", "JBL Guardada com sucesso!")
...
function message(player, text, type)
    exports['FR_DxMessages']:addBox(player, text, type)
end
