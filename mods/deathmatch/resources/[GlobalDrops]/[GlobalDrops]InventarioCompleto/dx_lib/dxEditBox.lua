...
local font = dxCreateFont("font/icons.ttf", 11, false, "proof")
...
function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	...
	if (#getElementData(element, "text2") == 0) then
		dxDrawText(text, x + 5, y, x + w - 10, y + h, tocolor(200, 200, 200, 200), 1, font, "center", "center", true, false, false)
	end
	if (dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font) <= w - 10) then
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 255), 1, font, "left", "center", true, false, false)
	else
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 255), 1, font, "right", "center", true, false, false)
	end
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("TS:EditBoxInv")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("TS:EditBoxInv")[editBoxId])) then
				if (getElementData(getElementsByType("TS:EditBoxInv")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("TS:EditBoxInv")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("TS:EditBoxInv")) do
		if (getElementData(element, "state") == true) then
