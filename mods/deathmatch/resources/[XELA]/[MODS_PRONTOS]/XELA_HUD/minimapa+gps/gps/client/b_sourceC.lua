		...
		if(gomb == "right" and statusz == "down" and nagyMapKattintas) then
			if(x > radarP[1] and x < radarP[1]+radarS[1] and y > radarP[2] and y < radarP[2]+radarS[2]) then
				if #gpsPontok == 0 then
					...
					--exports.btc_chat:sendLocalMeMessage(localPlayer, "Marcou no GPS um local")
...
--[[
	...
	for k,v in ipairs(getElementsByType ("blip")) do
        ...
        dxDrawImage ( b_x-blipMeret[1]/2,b_y-blipMeret[2]/2,blipMeret[1],blipMeret[2], "tex/blip/"..blip_icon..".png", 0, 0, 0, tocolor(bcR, bcG, bcB, 255))
    ...
    local jb_x = (((((3000)+jbx)/(6000)*(radarM[1]))-((radarS[1]/2)*zoom)-e1)+((radarS[1]/2)*zoom))/zoom
	local jb_y = ((((3000-jby)/(6000)*radarM[2])-((radarS[2]/2)*zoom)-e2)+((radarS[2]/2)*zoom))/zoom
	dxDrawImage ( jb_x-blipMeret[1]/2, jb_y-blipMeret[2]/2, blipMeret[1], blipMeret[2], "tex/blip/2.png", -rot, 0, 0, tocolor(255,255,255,255) )
	dxSetRenderTarget()
