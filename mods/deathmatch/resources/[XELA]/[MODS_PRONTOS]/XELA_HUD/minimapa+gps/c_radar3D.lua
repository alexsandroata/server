...
-- window settings
--local windowSize = {190, 118}

local windowSize = {375, 128}
local blipSize = 0.3
...
windowSize = {windowSize[1] * (scy / 1280) , windowSize[2] * (scy / 720)}
local windowStart = {scx * 0.029, scy * 0.95 - windowSize[2]}
...
local camHeight = {250, 455} -- #1 for ped #2 for veh
local camAngle = {-90, -44} -- #1 for ped #2 for veh
local arrowSize = 20 -- player arrow
...
hudMaskShader = dxCreateShader("fx/mask.fx")
maskTexture1 = dxCreateTexture("radar_mask.png")
...
addEventHandler("onClientResourceStart", resourceRoot, function()
	...
	radarTab.renderTarget = dxCreateRenderTarget(windowSize[1], windowSize[2], true)
	target_2 = dxCreateRenderTarget(windowSize[1], windowSize[2], true)
	if radarTab.enabled and radarTab.renderTarget then
		...
		radarTab.line = dxCreateShader("fx/image3D_manual_line.fx")
...
function loadGPS()
	...
	--setElementData(IDVitima, "SEQUESTRANDO:Vitima", false)
	...
	for k, v in ipairs(gpsPontok) do
		...
		dxSetShaderValue( radarTab.quad, "sElementPosition", v.x, v.y, 0 )
		dxSetShaderValue( radarTab.quad, "sElementSize", 15, 15 )
		dxSetShaderValue( radarTab.quad, "sTexColor", radarTab.blips[0] )	
		dxDrawImage( 0, 0, scx, scy, radarTab.quad, 0, 0, 0, tocolor(124, 197, 118) )	
...
--[[
	...
	-- skip to original RT
	dxSetRenderTarget()	
	dxSetRenderTarget(target_2, true)
	--dxDrawRectangle(0, 0, windowSize[1], windowSize[2], tocolor(140, 166, 214, 160))

	dxDrawImageSection( 0, 0, windowSize[1], windowSize[2],
                          0, 0, windowSize[1], windowSize[2], radarTab.renderTarget, 0, 0, 0, tocolor(255, 255, 255, 200) )
	dxSetRenderTarget()
	dxSetShaderValue( hudMaskShader, "sMaskTexture", maskTexture1 )
	dxSetShaderValue( hudMaskShader, "sPicTexture", target_2 )

	
	--dxDrawRectangle (windowStart[1]-5.5, windowStart[2]-5.7, windowSize[1]+10, windowSize[2]*1.2/1, tocolor ( 0, 0, 0, 155 ) )
	dxDrawImage(windowStart[1], windowStart[2], windowSize[1], windowSize[2], hudMaskShader)
	...
	for k, v in ipairs(getElementsByType("blip")) do
		...
		if bliPos[1] and bliPos[2] then
			...
			if actualDist <= 530 and getElementDimension(v) == plDimension and getElementInterior(v) == plInterior then
				local bid = getBlipIcon(v)	 --getElementData(v, "customIcon") or getBlipIcon(v)		
				local _, _, _, bcA = getBlipColor(v)
				local bcR, bcG, bcB = 255, 255, 255
				if getBlipIcon(v) == 0 then
					bcR, bcG, bcB = getBlipColor(v)
				end
				local bS = getBlipSize(v) * 0.4
...
--[[
			...
			if bliPos[1] and bliPos[2] then
				...
				if actualDist <= maxDist and v.dimension == plDimension and v.interior == plInterior then
					if isElement(v.texture) then
						...
						dxSetShaderValue( radarTab.blip, "sElementPosition", bliPos[1], bliPos[2], 0 )
						dxSetShaderValue( radarTab.blip, "sElementSize", bS, bS )
						dxDrawImage( 0, 0, 10, 10, radarTab.blip, 0, 0, 0, v.color )
					end
				end
			end
		end
	...
	--dxDrawRectangle(windowStart[1] + 16, windowStart[2] + windowSize[2] - addSize + 12, windowSize[1] + 2 * addSize - 79, 15, tocolor(90, 90, 90, 255))
...
--local scx, scy = guiGetScreenSize()
local teste = 1
dxDrawRectangle (windowStart[1]/1,  windowStart[2]*2.48/2, windowSize[1]/2.05, windowSize[2]/11, tocolor ( 0, 0, 0, 150 ) )
dxDrawRectangle (windowStart[1]/1,  windowStart[2]*2.48/2, windowSize[1]*health/205, windowSize[2]/11, tocolor ( 41, 167, 72, 165 ) )
dxDrawRectangle (windowStart[1]*3.84/1,  windowStart[2]*2.48/2, windowSize[1]/2.05, windowSize[2]/11, tocolor ( 0, 0, 0, 150 ) )
dxDrawRectangle (windowStart[1]*3.84/1,  windowStart[2]*2.48/2, windowSize[1]*armor/205, windowSize[2]/11, tocolor ( 1, 51, 151, 165 ) )
]]--

end
--addEventHandler("onClientRender", root, loadGPS)
...
function onVehicleExit(thePlayer, seat)
	if (thePlayer==getLocalPlayer()) then
		--if (seat<2) then
			--local id = getElementModel(source)
			--if not(enginelessVehicle[id]) then
				removeEventHandler("onClientRender", root, loadGPS)
			--end
		--end
	end
end
