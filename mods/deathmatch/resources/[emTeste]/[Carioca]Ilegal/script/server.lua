        ...
        if not isGuestAccount(getPlayerAccount(source)) then
            if isElementWithinMarker(source, MarkerIniciarRota[i]) then
                ...
                if isObjectInTableACLGroups(source, Acls) then
					...
					exports["[Carioca]Infobox"]:addBox(source, "Sem Permissão", "error")
                end
            end
        end
    end)
...
function()
   ...
   -- triggerClientEvent( source, 'Notify', source, "importante", "Você Finalizou a Rota com Sucesso")
	exports["[Carioca]Infobox"]:addBox(source, "Você Finalizou a Rota com Sucesso", "info")
end)
...
function ContinuarTrabalho(player)
    ...
    local vehicle = getPedOccupiedVehicle(player)
	    if ( vehicle ) then  exports["[Carioca]Infobox"]:addBox(source, "Você Finalizou a Rota com Sucesso", "info") return end
            ...
            if Index == #Config.PosicoesRotas[IndexRota] then
                ...
                setTimer(function()
                    ...
                    if BlipRota[player] and isElement(BlipRota[player]) then
						...
						exports["[Carioca]Infobox"]:addBox(source, "Você Entregou uma Caixa Prossiga para a Próxima", "info")
    ...
    if ResultComponent == true then
        if getPlayerMoney(source) >= Config.ConfigRotas["Dinheiro Fabricar"] then
            if not Fabricando[source] then
				...
				exports["[Carioca]Infobox"]:addBox(source, "Você Já Está Fabricando uma Droga", "info")
            end
        else
            --triggerClientEvent( source, 'Notify', source, "importante", "Sem Dinheiro Suficiente")
			exports["[Carioca]Infobox"]:addBox(source, "Sem Dinheiro Suficiente", "error")
        end
    else
        --triggerClientEvent( source, 'Notify', source, "importante", "Você Precisa dos Componentes ["..ResultComponent.."]")
		exports["[Carioca]Infobox"]:addBox(source, "Você Precisa dos Componentes ["..ResultComponent.."]", "error")
    end
end)
