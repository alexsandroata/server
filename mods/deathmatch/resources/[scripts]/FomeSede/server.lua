...
function checkFomeCount()
	for i, player in ipairs(getElementsByType("player")) do
		...
		if not isGuestAccount(acc) then
			...
			if fome <= 10 then
				exports["FR_DxMessages"]:addBox(player, "Você precisa se alimentar.", "info")
			end
			if fome <= 0 then
...
setTimer(checkFomeCount,120000,0)
...
function checkSedeCount()
	for i, player in ipairs(getElementsByType("player")) do
		...
		if not isGuestAccount(acc) then
			...
			if sede <= 10 then
				exports["FR_DxMessages"]:addBox(player, "Você precisa beber algo.", "info")
			end
			if sede <= 0 then
...
setTimer(checkSedeCount,100000,0)
