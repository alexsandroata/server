        ...
        for i, v in ipairs(Markers) do
            if getElementData(v, "NC.MarkerDroga") then
                if getElementData(localPlayer, "NC.Rota") then
                    ...
                    dxDrawTextOnElement(v, "#ffffffPRESSIONE #424949"..string.upper(Config.ConfigRotas["Tecla Iniciar Rota"]).." #ffffffPARA FINALIZAR A ROTA", 1.5, 15,255,255,255,255, 1.0)
                else
                    dxDrawTextOnElement(v, "#ffffffPRESSIONE #424949"..string.upper(Config.ConfigRotas["Tecla Iniciar Rota"]).." #ffffffPARA INICIAR A ROTA", 1.5, 15,255,255,255,255, 1.0)
                end
            end
        end
    end
...
function MensagemnaTela()
         dxDrawImage(x*842, y*976, x*235, y*54, "assets/img/rotas.png", 0, 0, 0, tocolor(255, 255, 255, Alpha), false)
        dxDrawText("Aperta  F7 para cancelar o destino\nVá até o destino e entregue tora.", x2*613+1, y2*725+1, x2*794+1, y2*700+1, tocolor(10, 10, 10), 1.0, font3, "left", "center", false, false, false, true, false)
        dxDrawText("Aperta  #424949F7#ffffff para cancelar o destino\nVá até o destino e entregue tora.", x2*613, y2*725, x2*794, y2*700, tocolor(255, 255, 255), 1.0, font3, "left", "center", false, false, false, true, false)
    end
...
function dxFabrica()
    ...
    for i = 1,#Posicoes do
        ...
        if #Drogas >= Index then
            ...
            dxDrawImage(x*Posicoes[i].Botao[1], y*Posicoes[i].Botao[2], x*Posicoes[i].Botao[3], y*Posicoes[i].Botao[4], "assets/img/botao.png", 0, 0, 0, (isCursorOnElement(x*Posicoes[i].Botao[1], y*Posicoes[i].Botao[2], x*Posicoes[i].Botao[3], y*Posicoes[i].Botao[4]) and tocolor(66, 73, 73, Alpha) or tocolor(66, 73, 73, Alpha)), false)
            ...
            dxDrawImage(x*posX, y*posY, x*posW, y*posH, ":MODInventario/gfx/itens/"..Diretorio..".png", 0, 0, 0, tocolor(255, 255, 255, 255), false)
            ...
            for k, v in ipairs(Drogas[Index].Componentes) do
                ...
                dxDrawImage(x*posX2, y*posY2, x*v.Tamanho[1], y*v.Tamanho[2], ":MODInventario/gfx/itens/"..v.ID..".png", 0, 0, 0, tocolor(255, 255, 255, Alpha), false)
            end
