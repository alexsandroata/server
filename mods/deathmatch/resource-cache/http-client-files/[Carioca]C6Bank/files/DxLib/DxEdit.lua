...
local font = dxCreateFont("files/FONTs/font_montserrat.ttf", 12)

outputDebugString('Sistema Bancario - Carioca Dev (Ativado com sucesso).')
outputDebugString('Nosso Site: https://carioca-group.blogspot.com/')
...
function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	...
	if (#getElementData(element, "text2") == 0) then
		dxDrawText(text, x + 5, y - 4, x + w - 10, y + h, tocolor(255, 255, 255, 180), 1.00, font, "center", "center", true, false, false)
	end
	if (dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font) <= w - 10) then
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 180), 1, font, "center", "center", true, false, false)
	else
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 180), 1, font, "right", "center", true, false, false)
	end
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("bankSystem")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("bankSystem")[editBoxId])) then
				if (getElementData(getElementsByType("bankSystem")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("bankSystem")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("bankSystem")) do
		if (getElementData(element, "state") == true) then
...
keyTable = {"mouse2", "mouse3", "mouse4", "mouse5", "mouse_wheel_up", "mouse_wheel_down", "arrow_l", "arrow_u",
...
function dxKeyElement(button, press)
	...
	if (press) and (button == "backspace") then	
		for _, element in ipairs(getElementsByType("bankSystem")) do
			if (getElementData(element, "state") == true) then
				if (#getElementData(element, "text2") > 0) then
					if getElementData(element, "text2") ~= key then
